...
--- Use inner join for departments and dept-manager Tables ((7.3.3))
...
-- using aliases for code readability ((7.3.3))
SELECT d.dept_name,
     dm.emp_no,
     dm.from_date,
     dm.to_date
...
---Use Left Join to capture retirement_info table((7.3.3))
...
--use aliases for code readability ((7.3.3))
SELECT ri.emp_no,
    ri.first_name,
ri.last_name,
    de.to_date
...
--using Left Join for retirment_ifo and dept_emp tables ((7.3.3))
SELECT ri.emp_no,
    ri.first_name,
    ri.last_name,
de.to_date
...
--using COUNT and GROUP BY ((7.3.4))
...
--using COUNT, GROUP BY, and ORDER BY ((7.3.4))
-- Employee count by department number
SELECT COUNT(ce.emp_no), de.dept_no
FROM current_emp as ce
LEFT JOIN dept_emp as de
...
-- adding/exporting the current_emp into csv file
